...
+NOTIFICATION_DB = "notifications.csv"
...
+    (CHAT_DB, ["Sender", "Receiver", "Message", "Timestamp"]),
+    (PERMISSION_DB, ["Requester", "Receiver", "Status"]),
+    (NOTIFICATION_DB, ["Receiver", "Type", "Source", "Content", "Timestamp", "Read"]),
+]:
...
+
+def send_new_message_email(receiver_email, sender_email, message):
+    msg = EmailMessage()
+    msg.set_content(f"You got a new message from {sender_email}:\n\n{message}")
...
+            server.send_message(msg)
+    except:
+        # Fail silently so app messaging never breaks
+        pass
+
+
...
+    mask = (df["Email"].astype(str).str.lower().str.strip() == email) & (
...
+    pd.DataFrame([[sender, receiver, message, ts]], columns=["Sender", "Receiver", "Message", "Timestamp"]).to_csv(
+        CHAT_DB, mode="a", header=False, index=False
+    )
+
...
+
+def add_notification(receiver, notif_type, source, content):
+    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
+    pd.DataFrame([[receiver, notif_type, source, content, ts, False]],
+                 columns=["Receiver", "Type", "Source", "Content", "Timestamp", "Read"]).to_csv(
+        NOTIFICATION_DB, mode="a", header=False, index=False
+    )
+
+
+def get_notifications(receiver_email):
+    try:
+        df = pd.read_csv(NOTIFICATION_DB)
+        user_df = df[df["Receiver"].astype(str).str.lower().str.strip() == receiver_email.lower().strip()]
+        return user_df.sort_values("Timestamp", ascending=False)
+    except:
+        return pd.DataFrame(columns=["Receiver", "Type", "Source", "Content", "Timestamp", "Read"])
+
+
+def mark_notifications_read(receiver_email, source_email=None):
+    try:
+        df = pd.read_csv(NOTIFICATION_DB)
+    except:
+        return
+
+    mask = df["Receiver"].astype(str).str.lower().str.strip() == receiver_email.lower().strip()
+    if source_email:
+        mask = mask & (df["Source"].astype(str).str.lower().str.strip() == source_email.lower().strip())
+
+    df.loc[mask, "Read"] = True
+    df.to_csv(NOTIFICATION_DB, index=False)
+
+
...
+    pd.DataFrame([[requester, receiver, "Pending"]], columns=["Requester", "Receiver", "Status"]).to_csv(
+        PERMISSION_DB, mode="a", header=False, index=False
+    )
     return True
...
-    
+
+    notif_df = get_notifications(st.session_state.user_email)
+    unread_df = notif_df[notif_df["Read"].astype(str).str.lower() != "true"] if not notif_df.empty else notif_df
+
+    with st.expander(f"🔔 Notifications ({len(unread_df)})", expanded=False):
+        if unread_df.empty:
+            st.info("No new notifications.")
+        else:
+            for _, nrow in unread_df.iterrows():
+                st.write(f"**{nrow['Type']}** from `{nrow['Source']}` at {nrow['Timestamp']}")
+                st.caption(nrow["Content"])
...
+                pd.DataFrame([[st.session_state.user_email, amt, cat]], columns=["Email", "Amount", "Category"]).to_csv(
+                    EXPENSE_DB, mode="a", header=False, index=False
+                )
